...
public class ElytraFlyPlusPlus extends Module {
    ...
    private final Setting<Boolean> lockYaw = sgGeneral.add(new BoolSetting.Builder()
        .name("lock-yaw")
        .description("Whether to lock your yaw when bounce is enabled. On by default so the auto diagonal snap is actually held.")
        .defaultValue(true)
        .visible(bounce::get)
        .build()
    );
    ...
    private final Setting<Double> yaw = sgGeneral.add(new DoubleSetting.Builder()
        .name("yaw")
        .description("The yaw to set when bounce is enabled. This is auto set to the closest diagonal (45/135/225/315) to you unless Use Custom Yaw is enabled. " +
    ...
    public ElytraFlyPlusPlus() {
        super(
            DWAddons.CATEGORY,
            "elytra-fly-plus-plus",
            "Elytra fly with some more features."
        );
    }

    /**
     * Whether Baritone is on the classpath. Only the obstacle passer needs it. Resolved via
     * {@link Class#forName} (no Baritone type referenced here) so this class loads — and the module
     * registers — even when Baritone is absent. All actual Baritone calls go through {@link EFlyBaritone}
     * and are guarded by this flag.
     */
    private static final boolean BARITONE_PRESENT = baritonePresent();

    private static boolean baritonePresent() {
        try {
            Class.forName("baritone.api.BaritoneAPI", false, ElytraFlyPlusPlus.class.getClassLoader());
            return true;
        } catch (Throwable t) {
            return false;
        }
    ...
    {
        // Only swallow server screen-closes during the fake-fly chestplate swap, which rapidly opens
        // the inventory to SWAP the elytra in. Cancelling EVERY close while the module is active
        // (the old behaviour) desynced any screen the server legitimately closed.
    ...
    public void onActivate()
    {
        ...
        {
            if (BARITONE_PRESENT && EFlyBaritone.elytraHasNoDestination())
            {
                EFlyBaritone.clearGoal();
            }
            ...
            if (!useCustomYaw.get())
            {
                ...
                {
                    double playerAngleNormalized = angleOnDiagonal(mc.player.getYaw());
                ...
                else
                {
                    ...
                    double angleNormalized = angleOnDiagonal(angle);
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (lastPos != null)
        {
            ...
            if (mc.player.isOnGround() && mc.player.isSprinting() && speedBps < speed.get())
            {
                ...
                {
                    // Original used ((IVec3d) event.movement).setY(0.0); that mixininterface method
                    // isn't present on this Meteor version, so reassign the field instead — same result
                    // (Y of the movement vector zeroed for this tick).
    ...
    private void onTick(TickEvent.Pre event)
    {
        ...
        if (bounce.get())
        {
            ...
            {
                tempPath = null;
                if (BARITONE_PRESENT) EFlyBaritone.clearGoal();
            }
            else if (tempPath != null)
            {
                if (BARITONE_PRESENT) EFlyBaritone.setGoalAndPath(tempPath);
                return;
            }

            // if still pathing, wait for that to complete
            if (highwayObstaclePasser.get() && BARITONE_PRESENT && EFlyBaritone.hasGoal())
            {
                return;
            }
            ...
            if (highwayObstaclePasser.get() && BARITONE_PRESENT && mc.player.getPos().length() > 100 && // > 100 check needed bc server sends queue coordinates when joining in first tick causing goal coordinates to be set to (0, 0)
                ...
                || stuckTimer > 50))
            {
                ...
                do
                {
                    ...
                    {
                        tempPath = goal;
                        EFlyBaritone.setGoalAndPath(goal);
                        return;
                    }
    ...
    // serverside uses default mappings.
    // 1.21.8 click packets carry hashed item predictions, not full stacks, so this is built with
    // ItemStackHash instead of the original Int2ObjectMap<ItemStack> constructor.
    ...
    // ── Highway-math helpers (the com.stash.hunt Utils functions, inlined) ─────────────────

    /** Snap an angle (deg) to the nearest diagonal heading (45/135/225/315), normalized to [0,360). */
    private static double angleOnDiagonal(double yawDeg) {
        double snapped = Math.round((yawDeg - 45.0) / 90.0) * 90.0 + 45.0;
