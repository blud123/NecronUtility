...
public class AutoElytraRestock extends Module {
    ...
    public AutoElytraRestock() {
        super(AddonTemplate.CATEGORY, "auto-elytra-restock",
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        switch (state) {
            ...
            case OPEN_CHEST -> {
                ...
                BlockPos target = mc.player.blockPosition().relative(facing);
                if (ec.found() && mc.level.getBlockState(target).isAir()) {
                    BlockUtils.place(target, ec, true, 50);
                    chestPos = target;
                }
                if (chestPos != null) {
                    Vec3 hit = Vec3.atCenterOf(chestPos);
    ...
    private int shiftPullFromContainer(java.util.function.Predicate<ItemStack> match, int max) {
        ...
        for (int i = 0; i < menu.slots.size() && done < max; i++) {
            ...
            if (slot.container == mc.player.getInventory()) continue; // skip player inventory
            if (!match.test(slot.getItem())) continue;
